...
def formatting_prompts_func(examples):
    ...
    routputs = examples["reasoning_advanced"]
    senseids = examples["senseid"]

    texts = []
    instruction = (
        "You are a linguistic assistant. Your task is word sense disambiguation. "
         "Given a sentence with an ambiguous word marked with <WSD> tags, determine its correct meaning. "
         "First, analyze the neighboring words and their influence. Then, apply deeper syntactic and semantic reasoning "
         "to decide on the correct senseID."
        )

    for input_text, output, routput, senseid in zip(inputs, outputs, routputs, senseids):
        output_t =  output + " Next, examine the syntactic and semantic context: "+ routput + " Taking both levels of reasoning into account, the correct senseID is " + senseid
...
dataset = load_dataset("Advance_Reasoning_for_Verb_WSD", split="train")
...
# Save the model
print("\nSaving model...")
model_name_hub = "finetuned-gemma-3-4b-it-WSD_VERB_hybrid"
