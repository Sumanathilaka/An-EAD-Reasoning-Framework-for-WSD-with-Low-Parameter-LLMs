...
def remove_wsd_tags(sentence: str) -> str:
    """
    Removes <WSD> and </WSD> tags from the sentence while keeping the word inside.

    Args:
        sentence (str): The sentence containing <WSD> tags.

    Returns:
        str: The sentence without the <WSD> tags.
    """
    return sentence.replace("<WSD>", "").replace("</WSD>", "")
...
print("Gemma 4b finetune model advance Reasoning loading...")

# Model configuration
model_name = "google/gemma-3-4b-it"
adapter_repo = "finetuned-gemma-3-4b-it-WSD_VERB_hybrid"
access_token = ""
...
def generate_text(prompt, word_amb, sentence):
    ...
    The model is prompted to resolve an ambiguous word using proper reasoning.
    """
    instruction = (
        "You are a linguistic assistant. Given a English sentence with an ambiguous word, "
        "identify its correct meaning using proper reasoning. All the outputs needs to be in English"
    )
    ...
    if response.startswith(full_prompt):
        ...
        # If prompt not matched, return full decoded response
...
def main():
    ...
    try:
        with open("updated_verb2.txt", "r", encoding="latin-1") as file:
            for line_num, line in enumerate(file, 1):
                try:
                    ...
                    sentence=remove_wsd_tags(sentence)
                    prompt = (
                        f"{wordw} is an ambiguous word in the sentence.\n"
                        f"Think step by step to identify the meaning of word {wordw}. Analyse both the syntactic role and the semantic influence that help in disambiguation. All outputs needs to be in English.\n\n"
